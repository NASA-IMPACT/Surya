...
REFERENCE_LOSS = np.array([0.24890851974487305, 0.31166040152311325, 0.32497961074113846, 0.32198894023895264, 0.3090871721506119, 0.3190929591655731, 0.36053016781806946, 0.33693042397499084])
SDO_CHANNELS = [
...
def batch_step(
    ...
    for t_idx in range(2):
        timestamp = np.datetime_as_string(batch_metadata["timestamps_input"][0][t_idx], unit="s")
    ...
    for step in range(0, rollout+1):
        if step == 0:
            ...
            curr_batch["ts"] = torch.cat(
                (curr_batch["ts"][:, :, 1:, ...], forecast_hat[:, :, None, ...]), dim=2
            )
        ...
        timestamp = np.datetime_as_string(batch_metadata["timestamps_targets"][0][step], unit="s")
        data_returned.append(
            SDOImage(
                channel="aia94",
                data=forecast_hat.to(dtype=torch.float32).cpu()[0, SDO_CHANNELS.index("aia94")].numpy(),
                timestamp=timestamp,
                type="output",
            )
        )
...
def test_surya_20140107(model: HelioSpectFormer, dataloader: DataLoader, caplog):
    ...
    for batch_idx, (batch_data, batch_metadata) in enumerate(dataloader):
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            with torch.no_grad():
                batch_loss, data_returned = batch_step(model, batch_data, batch_metadata, device, rollout=1)
    ...
    for j in range(8):
        for i in range(4):
                ...
                ax[j, i].set_title(f"{plot_data[j][i].type.capitalize()} - {plot_data[j][i].timestamp}")

    fig.suptitle(f"Surya - AIA94", y=0.9)
    plt.savefig(f"surya_model_validation.png", dpi=120, bbox_inches="tight")
